void getNreg() {
  ...
  if (dow == 1) matrix.drawRGBBitmap(56, 25, image_data_BLUE, 5, 3);
    ...
    if ((h == hof - 1) || (h == hof && m <= minof)) matrix.setTextColor(matrix.Color333(1, 0, 0));
     if (m < 10) {
      ...
      matrix.setFont(&fonts[fch]);
      if (s % 2 == 0) {
      if (modtoch == false){matrix.fillRect(30, 13, 4, 2, myBLACK);matrix.swapBuffers(true);}
      if (modtoch == true){matrix.println(":");matrix.swapBuffers(true);}                        //  {matrix.fillRect(30, 12, 4, 2, myRED);matrix.fillRect(30, 18, 4, 2, myRED);matrix.swapBuffers(true);}
    }
      if (s % 2 != 0) {
      if (modtoch == false){matrix.fillRect(30, 17, 4, 2, myBLACK);matrix.swapBuffers(true);}
    }
     matrix.setFont();
     matrix.swapBuffers(true); 
     getper();
      ...
      if (((month == 3 || month == 5 || month == 8 || month == 10) && day == 30) || ((month == 0 || month == 2 || month == 4 || month == 6 || month == 7 || month == 9 || month == 11) && day == 31) || ((month == 1) && day == 28))
  {
   ...
   if(m % 2 == 0 && s == 0)getwifi();
    updateTime();
   if((hon < hof) && (h < min(hon, hof) || h > max(hon, hof+1))){getPilik();FlagNr = 0;Y = W;matrix.fillRect(0, 0, 64, 32 , myBLACK);matrix.setFont();getRTCh(szTime);getRTCm(szTime);}
   if((hon > hof) && (h > min(hon, hof) && h < max(hon, hof+1))){getPilik();FlagNr = 0;Y = W;matrix.fillRect(0, 0, 64, 32 , myBLACK);matrix.setFont();getRTCh(szTime);getRTCm(szTime);}
   if (h == hof && m == minof && s == 00){getPilik();FlagNr = 0;Y = W;matrix.fillRect(0, 0, 64, 32 , myBLACK);delay(100);matrix.setFont();getRTCh(szTime);getRTCm(szTime);getYtro();}
  } while (FlagNr == 1 && MNR == true);
  matrix.setFont();
  matrix.fillRect(0, 0, 64, 32 , myBLACK);

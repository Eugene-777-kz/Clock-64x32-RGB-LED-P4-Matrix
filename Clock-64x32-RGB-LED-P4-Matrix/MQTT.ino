...
void reconnect() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    while (!client.connected()) {
      ...
      String clientId = "esp32Matrix-Clock-";
      ...
      if (client.connect(clientId.c_str(), MQTT_USER, MQTT_PASSWORD)) {
        ...
        if (RECEIVER_CH1 != 0)client.subscribe(MQTT_SERIAL_RECEIVER_CH1);
        if (RECEIVER_CH2 != 0)client.subscribe(MQTT_SERIAL_RECEIVER_CH2);
        if (RECEIVER_CH3 != 0)client.subscribe(MQTT_SERIAL_RECEIVER_CH3);
        if (RECEIVER_CH4 != 0)client.subscribe(MQTT_SERIAL_RECEIVER_CH4);
...
void publmqtt()
{
  if (Serv == true) {
    ...
    switch (wetu) {
      ...
      case 2:
        if (POG == 0) {
          client.publish(MQTT_SERIAL_PUBLISH_CH2, String((location_pres), 0).c_str());
        }
        if (POG == 1) {
          client.publish(MQTT_SERIAL_PUBLISH_CH2, String((pressures / 1.355269), 0).c_str()); //
        }
        if (POG == 2) {
          client.publish(MQTT_SERIAL_PUBLISH_CH2, String((pressuredarc / 1.33372), 0).c_str());
        }
        break;
      case 3:
      case 4:
    ...
    if (Mod1 == true) {
      client.publish(MQTT_SERIAL_PUBLISH_CH7, String(mySensor.CO2).c_str());
      client.publish(MQTT_SERIAL_PUBLISH_CH6, String(mySensor.TVOC).c_str());
    }

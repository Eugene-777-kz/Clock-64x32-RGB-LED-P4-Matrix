...
const char* myFilePath = "/config.txt";
Adafruit_AHTX0 aht;
...
unsigned long timing, timeNar, LostWiFiMillis, lastUpd, lastUpd1, lastUpd2, lastDay, timePog, timeNews, timePog1, timePog2; //Переменные для хранения точек отсчета
unsigned long TimeShow;
unsigned long TimeShow1;
unsigned long TimeShow2;
unsigned long TimeShow3;
TimerMs tmr(10000, 1, 1);
TimerMs tmr1(2000, 1, 1);
TimerMs tmr2(500, 1, 1);
TimerMs tmr3(200, 1, 0);
TimerMs tmr4(200, 1, 0);
TimerMs tmr5(1000, 1, 1);
...
int httpCode;
int x = 4, x1 = 5, x2 = 8, x3 = 0, x4 = 1, x5 = 6, x6 = 0, x7 = 4, e = 0, x8 = 4, x21 = 8, x22 = 8, x23 = 8, x24 = 8, x25 = 8, x26 = 8, x27 = 8, x28 = 8, x29 = 6, xob, x30 = 5, x31 = 0, x32 = 1, x33 = 2, x34 = 0, x35 = 4, x36 = 1, x37 = 2, x39 = 6, x40 = 5, x41 = 0, x42 = 6, x43 = 5, x44 = 0, x45 = 5, x46 = 0, x47 = 0, x48 = 0, xnews = 3, xchut = 4, xvalu = 2;
...
uint16_t myShadow = matrix.Color333(1, 0, 3);
uint16_t myROSE = matrix.Color333(3, 0, 1);
...
uint16_t myShadow1 = matrix.Color333(2, 0, 4);
uint16_t myROSE1 = matrix.Color333(4, 0, 2);
...
uint16_t myShadow2 = matrix.Color333(2, 0, 4);
uint16_t myROSE2 = matrix.Color333(4, 0, 2);
...
uint16_t myShadow3 = matrix.Color333(3, 0, 5);
uint16_t myROSE3 = matrix.Color333(5, 0, 3);
...
uint16_t myShadow4 = matrix.Color333(3, 0, 6);
uint16_t myROSE4 = matrix.Color333(6, 0, 3);
...
uint16_t myShadow5 = matrix.Color333(3, 0, 7);
uint16_t myROSE5 = matrix.Color333(7, 0, 3);
...
//uint16_t myCOLORA[][6] = {{myRED1, myGREEN1,  myBLUE1, myYELLOW1, myCYAN1, myORANGE1},{myRED, myGREEN, myBLUE, myYELLOW, myCYAN, myORANGE},{myRED2, myGREEN2, myBLUE2, myYELLOW2, myCYAN2, myORANGE2}};
...
int Mgl = 23;
//bool ModPl = true;
bool Modebst1, Modebst2, Modebst3, Serv, Modmqtt, Modekr1, Modekr2, Modekr3, Modebst4, Modenews, Modenewspod, Modewebr, flagpr;
bool Modebst5 = true;
int modfch = 0;
int modfss = 0;
int fch = 0;
...
const char*Moddat3[] = {"NO",  "BME280", "AHTXX", "NM", "GY21",};//влажность//wetyint weti;
int weti;
int wety;
int wetu;
int HUM = 40;
...
bool flagval = true;
bool flagP;
int hwebon = 20, mwebon = 00, hweboff = 21, mweboff = 00;
int fytro = 0;
bool flagzapros = false;
// переменные

String gorod, lat, lon, weatherKey0, uuid, api_key;
char mqtt_server[25] = "";
int  mqtt_port;
char MQTT_USER[16] = "";
...
//String KEY = "";//ключ...с https://exchangeratesapi.io///Cp2Q2hRdMbXzilOmOddFXtVFKzBPwoX1
int val1;// основная валюта
int val2;
float valute1;
float valute2;
String valosn;
...
{
  xob = x22;
  if (Modebst1 == true && flagrtc == 0 && ModWEBR != 2 && modWR != 1 && kch == true && flagzapros == true)scroll_text(24, framflag = frameDelay, (weatherString));
  pogflag = false;
  flagMap = false;
  flagDarc = false;
}
void getStr5()
{
  if (m < 55 && Modmqtt == false && Serv == true && ModWEBR != 2 && modWR != 1 && (file1 != 0 || file2 != 0 || file3 != 0 || file5 != 0 || file6 != 0)) {
    xob = x28;
    scroll_text(24, framflag = frameDelay5, (" MQTT  " + mqttstring));
  }
}
void getStr6()
{
  xob = x23;
  if (Modebst2 == true && flagrtc == 0 && ModWEBR != 2 && modWR != 1 && kch == true && flagzapros == true)scroll_text(24, framflag = frameDelay1, (weatherStringZ));
  pog_forflag = false;
  flagMap = false;
  flagDarc = false;
}
void getStr7()
{
  if (flagrtc == 1)scroll_text2(24, framflag = frameDelay + 10, ("Нет wi-fi!!!"));
}
void getStr8()
{
  if (NarFlag == 1 && Modebst3 == true && flagrtc == 0 && ModWEBR != 2 && modWR != 1) {
    xob = x24;
    if (tempNM > 0) {
      texttempNM = "+" ;
...
void getStr9()
{
  xob = x25;
  if (file4 != 0) {
    scroll_text(24, framflag = frameDelay5, ("Текстовое сообщение - " + String(file4)));
  }
}
void getSens()
{
  ...
  if (weti == 1) {
    getsensorsBme();
    Domtemhumpres += String(tempBme + CorTemp, 0);
  }
  if (weti == 2) {
    getsensorsAth();
    sensors_event_t temp;
    aht_temp->getEvent(&temp);
    Domtemhumpres += String(temp.temperature + CorTemp, 0);
  }
  if (weti == 3) {
    sensorsBmp280();
    Domtemhumpres += String(tempBmp + CorTemp, 0);
  }
  if (weti == 4) {
    sensorsBmp180();
    Domtemhumpres += String(tempBmp + CorTemp, 0);
  ...
  if (weti == 5) {
    getsensGY21();
    Domtemhumpres += String(htu.getTemperature() + CorTemp, 0);
  }
  ...
  if (wety == 2) {
    ...
    Domtemhumpres += String(humidity.relative_humidity + CorHum, 0);
  }
  if (wety == 3)Domtemhumpres += String(humNM , 0);
  if (wety == 4) {
    getsensGY21();
    Domtemhumpres += String(htu.getHumidity(), 0);
  }
  if (wety == 1 || wety == 2 || wety == 3 || wety == 4)Domtemhumpres += "%,";
  if (wetu == 1 || wetu == 2 || wetu == 3 || wetu == 4)Domtemhumpres += " давление: " ;
  if (wetu == 1) {
    getsensorsBme();
    Domtemhumpres += String(pressBme + corrPress, 0);
  }
  ...
  if (wetu == 3) {
    sensorsBmp280();
    Domtemhumpres += String(pressBmp + corrPress, 0);
  }
  if (wetu == 4) {
    sensorsBmp180();
    Domtemhumpres += String(pressBmp + corrPress, 0);
  }
  if (wetu == 1 || wetu == 2 || wetu == 3 || wetu == 4)Domtemhumpres += " ммРс";
  if (FlagNr != 1 && Modebst4 == true) {
    xob = x26;
    scroll_text(24, framflag = frameDelay6, (Domtemhumpres));
  }
}
void getStr10()
{
  if (Modsen == 1 && FlagNr != 1) {
    ...
    if (tempds != -127) {
      if (tempds > 0) {
        TextU = "+";
        scroll_text2(24, frameDelay + 10, ("Улица:" + TextU + String(tempds + cords18, 0) + "\260"));
      } else {
        scroll_text2(24, frameDelay + 10, ("Улица:" + String(tempds + cords18, 0) + "\260"));
      }
    } else {
...
void getStr11()
{
  if (Text != 0) {
    xob = x27;
    scroll_text(24, framflag = frameDelay3, (Text));
  }
...
{
  if (Modewebr == true) {
    xob = x48; scroll_text3(24, framflag = frameDelay10, (webRadio1 + webRadio));
  } else {
    xob = x48; scroll_text3(24, framflag = frameDelay10, (segodnya));
  }

}
void getStr13()
{
  if (FlagNr != 1 && Modenews == true && location_news != 0) {
    xob = xnews;
    rep();
    repl();
    scroll_text(24, framflag = frameDelay8, ("Новости : " + nowosti));
  }
  newsflag = false;
...
void getStr15()
{
  ...
  if (FlagNr != 1 && chut == true && location_anek != 0) {
    xob = xchut;
    repl();
    scroll_text(24, framflag = frameDelay11, (aforizm));
    chutflag = false;
  }
}
void getStr16()
{
  valyte();
  flagstring = false;
  if (FlagNr != 1 && valu == true && valutedok !=  0) {
    xob = xvalu;
    scroll_text(24, framflag = frameDelay12, (valutedok));
  }
  flagval = false;
}
void getStr17()
{
  scroll_text(24, framflag = frameDelay9, (""));
}
void rep()
{
  nowosti.reserve(500);
  ...
  switch (np) {
    case 0: nowosti = location_news; break;
    case 1: nowosti = location_news1; break;
    case 2: nowosti = location_news2; break;
    case 3: nowosti = location_news3; break;
    case 4: nowosti = location_news4; break;
    case 5: nowosti = location_news5; break;
    case 6: nowosti = location_news6; break;
    case 7: nowosti = location_news7; break;
    case 8: nowosti = location_news8; break;
    case 9: nowosti = location_news9; break;
  }
}
void repl()
{
...
void getDate1()
{
  ...
  if (ModWEBR != 2) {
    ...
    matrix.setFont(&fontss[1]);
    matrix.setTextColor(myCOLORS[e][x48]);
    matrix.setCursor(6, 6);
    matrix.print("WEBRADIO");
    matrix.setFont();
  }
}
...
void getTim(char *psz, bool f)
{
  ...
  if (s % 2 == 0) {
    ...
    if (modtoch == true) {
      matrix.fillRect(22, 11, 4, 2, myBLACK);
      matrix.fillRect(22, 17, 4, 2, myBLACK);
    }
  }
  if (s % 2 != 0) {
  ...
  sprintf(psz, "%02d", s);
  matrix.fillRect(47, 16, 17, 7, myBLACK);//
...
void ysl()
{
  ...
  if (h % 2 == 0 && m == 15 && s == 00)newsflag = true;
  if (h == hon && m == minon && MNR == true) {
    FlagNr = 1;
    getNreg();
  }
  if ((hon < hof) && (h > min(hon, hof) && h < max(hon, hof - 1)) && MNR == true) {
    FlagNr = 1;
    getNreg();
  }
  if ((hon > hof) && (h > max(hon, hof) || h < min(hon, hof - 1)) && MNR == true) {
    FlagNr = 1;
    getNreg();
  }
  if ((h == hbigon && m == mbigon) && (BCH == true)) {
    getPilik();
    modBig = 1;
    getBigTime();
  }
  if (m % 5 == 0 && s == 00)flagval = true;
  ...
  if (!client.connected() && Serv == true && flagstring == true)reconnect();
  if (Modbyd1 == true || Modbyd2 == true || Modbyd3 == true || Modbyd4 == true || Modbyd5 == true || Modbyd6 == true || Modbyd7 == true) {
    getper();
    getprbud();
  }
...
void scroll_text(uint8_t ypos, unsigned long scroll_delay, String text)
{
  ...
  if (Serv == true && ModWEBR != 2 && modWR != 1)zppMqtt();
  flagstring = true;
}
...
void scroll_text3(uint8_t ypos, unsigned long scroll_delay, String text)
{
  uint16_t text_length = text.length();
  for (int xpos = matrix_width; xpos > -(matrix_width + text_length * 5); xpos--)
  {
...
void getYtro() {
  ...
  do {
    ...
    if (fytro == 1 && millis() - TimeShow2 > 200) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 2 && millis() - TimeShow2 > 300) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 3 && millis() - TimeShow2 > 300) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 4 && millis() - TimeShow2 > 200) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 5 && millis() - TimeShow2 > 100) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 6 && millis() - TimeShow2 > 100) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 7 && millis() - TimeShow2 > 10) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
    ...
    if (fytro == 8 && millis() - TimeShow2 > 10) {
      TimeShow2 = millis();
      matrix.drawRGBBitmap(0, 0, image_data_eyes, 64, 32);
      matrix.swapBuffers(true);
      fytro++;
    }
    if (fytro == 9 && millis() - TimeShow2 > 300) {
      TimeShow2 = millis();
      if (MG == 3) {
        dfPlayer.volume(0);
      } else {
    ...
    if (fytro == 10 && millis() - TimeShow2 > 300) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes2, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 11 && millis() - TimeShow2 > 300) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes3, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 12 && millis() - TimeShow2 > 200) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes4, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 13 && millis() - TimeShow2 > 100) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes5, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 14 && millis() - TimeShow2 > 100) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes6, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 15 && millis() - TimeShow2 > 100) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes7, 64, 32);
      matrix.swapBuffers(true);
    ...
    if (fytro == 16 && millis() - TimeShow2 > 1000) {
      TimeShow2 = millis();
      matrix.fillScreen(0);
      matrix.drawRGBBitmap(0, 0, image_data_eyes, 64, 32);
      matrix.swapBuffers(true);
...
void setup() {
  ...
  Serial.begin(115200);
  xTaskCreatePinnedToCore(Task1code, "Task1", 8192, NULL, 15, NULL, 0);//xTaskCreatePinnedToCore(Task1code, "Task1", 10000, NULL, 15, &Task1, 0);
  delay(500);
...
void vklweb()
{
  if (WEBR == true && h == hwebon && m == mwebon && s == 00) {
    getPilik();
    St();
    ModWEBR = 2;
    modWR = 1;
  }
  if (WEBR == true && h == hweboff && m == mweboff && s == 00) {
    getPilik();
    modWR = 0;
    ModWEBR = 0;
    Serial.println("stop");
...
void Task1code( void * pvParameters ) {
  for (;;) {
    if (ModWEBR == 2 && modWR == 1 && WiFi.status() == WL_CONNECTED) {
      portal.tick();
      audio.loop();
    }
    delay(1);
...
void loop() {
  getwifi();
  if (ModWEBR == 0 && modWR == 0) {
    ArduinoOTA.handle();
    getDate();
    getHome();
  } else {
    if (flagrtc == 0)getStr12();
  }
  if (modBig == 1 && BCH == true)getBigTime();
  if (ModWEBR == 0 && modWR == 0)getStr2();
  if (ModWEBR == 0 && modWR == 0)getStr3();
  if (ModWEBR == 0 && modWR == 0 && Mod1 == 1 && Modekr3 == true && (m < 55 || m == minon - 1)) getNow2();
  if (ModWEBR == 0 && modWR == 0 && pogflag == true && kch == true && POG == 0 )getWeatherData0();
  if (ModWEBR == 0 && modWR == 0 && flagMap == true && kch == true && POG == 1)WetMap();
  if (ModWEBR == 0 && modWR == 0 && flagDarc == true && kch == true && POG == 2)Meteo();
  if (ModWEBR == 0 && modWR == 0)getStr4();
  if (ModWEBR == 0 && modWR == 0 && (m < 55 || m == minon - 1) && Modmqtt == true && Serv == true && (file1 != 0 || file2 != 0 || file3 != 0 || file5 != 0 || file6 != 0))getNow4();
  if (ModWEBR == 0 && modWR == 0)getStr5();
  if (ModWEBR == 0 && modWR == 0 && pog_forflag == true && kch == true && POG == 0)getWeatherDataz0();
  if (ModWEBR == 0 && modWR == 0)getStr6();
  if (ModWEBR == 0 && modWR == 0 && chut == true && chutflag == true) {
    Anek();
    getStr15();
  }
  if (flagrtc == 1)getStr7();
  if ((ModWEBR == 0 && modWR == 0 && Modekr2 == true && (m < 55 || m == minon - 1)) && (M1 == true || M2 == true || M3 == true || M4 == true || M5 == true))getNow1();
  if (ModWEBR == 0 && modWR == 0 && nmonflag == true)getNarodmon();
  if (ModWEBR == 0 && modWR == 0)getStr8();
  if (ModWEBR == 0 && modWR == 0)getStr9();
  if (ModWEBR == 0 && modWR == 0 && Modekr1 == true && (m < 55 || m == minon - 1) && flagrtc == 0 && flagzapros == true)getNow();
  if (ModWEBR == 0 && modWR == 0)getSens();
  if (ModWEBR == 0 && modWR == 0)getStr10();
  if (ModWEBR == 0 && modWR == 0)getStr11();
  if (ModWEBR == 0 && modWR == 0 && newsflag == true && Modenews == true)News();
  if (ModWEBR == 0 && modWR == 0)getStr13();
  if (ModWEBR == 0 && modWR == 0 && valu == true && flagval == true)Val();
  if (ModWEBR == 0 && modWR == 0)getStr16();
  if (ModWEBR == 0 && modWR == 0) {
    np++;
    if (np > 9)np = 0;
    if (FlagNr == 0) {
